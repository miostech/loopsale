...
import { normalizeHotmartPayloadWithStarted } from "@/lib/webhooks/normalize";
import { processIncomingEvent } from "@/lib/webhooks/process-event";

export async function POST(request: Request) {
  try {
    const id = getClientIdentifier(request);
    const limit = checkRateLimit(`hotmart:${id}`);
    if (!limit.ok) {
    ...
    const normalized = normalizeHotmartPayloadWithStarted(body);
    if (!normalized) {
      return NextResponse.json(
    ...
    const allIntegrations = await integrationsCol
      .find({ platform: "hotmart", active: true })
      .toArray();
